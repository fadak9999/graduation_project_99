...
//               SharedPreferences prefs = await SharedPreferences.getInstance();
//               await prefs.clear();
//               Get.offAllNamed('/login');  // إغلاق كافة الشاشات والانتقال إلى صفحة تسجيل الدخول
//             },
//             child: Text(
...
class _AccountState extends State<Account> {
  ...
  Future<void> _loadSettings() async {
    ...
    // قراءة الاسم المحفوظ
    String? savedName = prefs.getString('userName');
    // قراءة مسار صورة البروفايل المحفوظ
    String? imagePath = prefs.getString('profileImage');
    
    print('Loaded language code: $languageCode');
    print('Loaded user name: $savedName');
    print('Loaded profile image path: $imagePath');
  ...
  Future<void> _saveProfileImage(String path) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    print('Saving profile image: $path');
    await prefs.setString('profileImage', path);
  }
...
Future<void> _pickImage() async {
  ...
  Future<void> _changeLanguage(Locale locale) async {
    ...
    SharedPreferences prefs = await SharedPreferences.getInstance();
    print('Saving language: ${locale.languageCode}');
    await prefs.setString('languageCode', locale.languageCode);
  }
Future<void> _confirmLogout() async {
  showDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.of(context).pop(); // Close the dialog
              SharedPreferences prefs = await SharedPreferences.getInstance();
              await prefs.clear();  // Clear stored data (if any)

              // Close all screens and navigate to the login page without ability to return
              Get.offAll(() => login());  // Redirect to login and remove all previous screens from the stack
            },
            child: Text(
              _locale?.languageCode == 'ar' ? 'تأكيد' : 'Confirm',
            ),
